...
from .topic_families import BOOTSTRAP_TOPIC, canonical_reserved_prefixes, generic_user_notify_external_topic


DEFAULT_RESERVED_PREFIXES: tuple[str, ...] = tuple(canonical_reserved_prefixes())
DEFAULT_BOOTSTRAP_TOPIC = BOOTSTRAP_TOPIC
GENERIC_USER_NOTIFY_EXTERNAL_TOPIC = generic_user_notify_external_topic()
...
class MqttAclCompiler:
    ...
    def _to_acl_text(self, state: MqttIntegrationState, rules: list[CompiledAclRule]) -> str:
        lines: list[str] = []
        lines.append("# generated by hexe core mqtt acl compiler")
        ...
        for principal_id in principal_ids:
            ...
            principal_rules = self._ordered_principal_rules(state, principal_id, rules)
    ...
    @staticmethod
    def _ordered_principal_rules(
        state: MqttIntegrationState,
        principal_id: str,
        rules: list[CompiledAclRule],
    ) -> list[CompiledAclRule]:
        principal_rules = [rule for rule in rules if rule.principal_id == principal_id]
        ordered = sorted(principal_rules, key=lambda item: (item.effect, item.action, item.topic))
        principal = state.principals.get(principal_id)
        if principal is None or principal.principal_type != "generic_user":
            return ordered
        access_mode = str(getattr(principal, "access_mode", "private") or "private").strip().lower()
        if access_mode == "admin":
            return ordered
        trailing_exception = [
            rule
            for rule in ordered
            if rule.effect == "allow" and rule.topic == GENERIC_USER_NOTIFY_EXTERNAL_TOPIC
        ]
        if not trailing_exception:
            return ordered
        remaining = [
            rule
            for rule in ordered
            if not (rule.effect == "allow" and rule.topic == GENERIC_USER_NOTIFY_EXTERNAL_TOPIC)
        ]
        return [*remaining, *trailing_exception]
